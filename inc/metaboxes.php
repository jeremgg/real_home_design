...
add_action('add_meta_boxes', '_rhd_add_custom_box');
/**
 * Add Meta Boxes.
 *
 * Add Meta box in page and post post types.
 */
function _rhd_add_custom_box()
{
    add_meta_box('siderbar-layout', //Unique ID
        __('Select layout for this specific Page only ( Note: This setting only reflects if page Template is set as Default Template and Blog Type Templates.)', '_rhd'), //Title
        '_rhd_sidebar_layout', //Callback function
        'page' //show metabox in pages
        );
    add_meta_box('siderbar-layout', //Unique ID
        __('Select layout for this specific Post only', '_rhd'), //Title
        '_rhd_sidebar_layout', //Callback function
    ...
    if ( class_exists( 'WooCommerce' ) ) {
        add_meta_box('product-siderbar-layout', //Unique ID
            __('Select layout for this specific Product only', '_rhd'), //Title
            '_rhd_sidebar_layout', //Callback function
...
add_action('save_post', '_rhd_save_custom_meta');
/**
 * save the custom metabox data
 * @hooked to save_post hook
 */
function _rhd_save_custom_meta($post_id)
{
    global $site_layout, $post;
